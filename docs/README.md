...
### 🚀 Быстрый старт
- [⚡ Быстрый старт на сервере](SERVER_QUICKSTART.md) - запуск за 5 минут
...
### 🆘 Решение проблем
- [🚨 СРОЧНОЕ исправление](IMMEDIATE_FIX.md) - команды Docker для немедленного решения
- [⚡ Быстрое исправление](QUICK_FIX.md) - ошибка миграции "type already exists"
- [🔧 Подробное руководство](migration_fix.md) - все варианты решения проблем с миграциями
- [📦 Имена Docker Volumes](VOLUME_NAMES.md) - как определить и удалить volumes
...
- Парсит транзакцию через AgentRouter API (модель GPT-5)
...
- **AgentRouter API** - парсинг транзакций через модель GPT-5
- **Alembic** - миграции БД
- **Loguru** - логирование
...
- [AgentRouter](https://agentrouter.org) - единый API-шлюз для доступа к LLM моделям (GPT-5)
